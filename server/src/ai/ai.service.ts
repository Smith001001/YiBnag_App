...
2. recommend_order - 用户想找订单接单（如："推荐订单"、"有什么单子"、"帮我找合适的订单"、"给我推荐订单"、"我想接单"、"我要接一单"、"有什么单可以接"）
3. accept_order - 用户指定要接哪个订单（必须包含具体序号，如："接第1单"、"我要接第2个订单"、"我想接第三个"、"接第2单"）
...
2. **接单意图识别（必须包含具体序号才是accept_order）**：
   - "接第1单"、"接第2个"、"我要接第三个" → intent: "accept_order", orderIndex: 对应数字
   - "我想接单"、"我要接一单"、"有什么单可以接" → intent: "recommend_order"（用户没指定具体订单，需要先推荐）
   - 只有用户明确说了"第X个"或"第X单"才识别为 accept_order
...
5. **意图识别规则**：
   ...
   - "取消" → intent: "cancel"
   - "修改" → intent: "modify"
   - "接第1单"/"接第2个"/"我要接第三个"（必须含具体序号） → intent: "accept_order", orderIndex: 对应数字
   - "我想接单"/"有什么单"/"推荐订单" → intent: "recommend_order"（用户想看订单列表）
